...
The interaction between the client player, the device sensors, and the Supabase backend is illustrated below.
\begin{figure}[H]
    \centering
    \begin{tikzpicture}[
        font=\scriptsize,
        actor/.style={draw, ellipse, minimum width=2.2cm, minimum height=0.62cm, align=center, fill=gray!8, line width=0.8pt},
        usecase/.style={draw, rectangle, minimum width=3.15cm, minimum height=0.52cm, align=center, fill=white, line width=0.75pt, text depth=0pt},
        service/.style={draw, ellipse, minimum width=2.45cm, minimum height=0.62cm, align=center, fill=blue!5, line width=0.75pt},
        storage/.style={draw, cylinder, shape border rotate=90, aspect=0.25, minimum width=1.8cm, minimum height=0.95cm, align=center, fill=cyan!8, line width=0.75pt},
        link/.style={-{Stealth[length=2mm]}, line width=0.7pt, draw=black!72}
    ]
        % Actors
        \node (player) [actor] at (-4.6, 0.2) {\textbf{Player}};

        % System Boundary
        \draw[gray!55, line width=0.8pt, rounded corners=5pt] (-2.35, 4.55) rectangle (2.35, -5.55);
        \node[fill=white, text=gray!75] at (0, 4.55) {\textbf{WhereAmI Android app}};

        % Use Cases
        \node (auth) [usecase] at (0, 3.85) {Register / log in};
        \node (social) [usecase] at (0, 3.05) {Manage friends};
        \node (groups) [usecase] at (0, 2.25) {Create and manage groups};
        \node (lobby) [usecase] at (0, 1.45) {Create game lobby};
        \node (capture) [usecase] at (0, 0.65) {Capture or select photo};
        \node (geotag) [usecase] at (0, -0.15) {Attach GPS coordinates};
        \node (upload) [usecase] at (0, -0.95) {Upload geotagged photo};
        \node (round) [usecase] at (0, -1.75) {View photo};
        \node (guess) [usecase] at (0, -2.55) {Place guess on map};
        \node (score) [usecase] at (0, -3.35) {Calculate score};
        \node (leaderboard) [usecase] at (0, -4.15) {View leaderboard};
        \node (profile) [usecase] at (0, -4.95) {Update profile and password};

        % External services and device sensors
        \node (camera) [service] at (4.45, 0.65) {Camera sensor};
        \node (gps) [service] at (4.45, -0.15) {Device location};
        \node (map) [service] at (4.45, -2.55) {OpenStreetMap};
        \node (supabase) [storage] at (4.55, -3.85) {\textbf{Supabase}};

        % Player Connections
        \foreach \uc in {auth,social,groups,lobby,capture,round,guess,leaderboard,profile}
            \draw [link] (player) -- (\uc.west);

        % Device Sensor Connections
        \draw [link] (capture.east) -- (camera);
        \draw [link] (geotag.east) -- (gps);
        \draw [link] (guess.east) -- (map);
        \draw [link] (capture) -- (upload);
        \draw [link] (geotag) -- (upload);
        \draw [link] (guess) -- (score);
        \draw [link] (score) -- (leaderboard);

        % Backend Connections
        \foreach \uc in {auth,social,groups,lobby,upload,round,guess,score,leaderboard,profile}
            \draw [link] (\uc.east) -- (supabase);
    \end{tikzpicture}
