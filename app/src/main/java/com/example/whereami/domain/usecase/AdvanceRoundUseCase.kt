...
class AdvanceRoundUseCase(
    ...
    suspend operator fun invoke(game: Game, finishedRound: Round, guesses: List<Guess>): Result<Game> {
        return try {
            ...
            val updatedRound = finishedRound.copy(status = com.example.whereami.domain.model.RoundStatus.FINISHED)
            ...
            val updatedGame = game.copy(
                ...
                rounds = game.rounds.map { r ->
                    if (r.id == finishedRound.id) {
                        ...
                        r.copy(startTime = newStartTime, endTime = newEndTime)
                    } else {
                        r
                    }
                }
